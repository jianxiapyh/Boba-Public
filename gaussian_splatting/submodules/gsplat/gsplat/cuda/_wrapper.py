...
@torch.no_grad()
def fully_fused_projection_shared_template(
    means: Tensor,  # [B, N, 3]
    quats: Tensor,  # [B, N, 4]
    template_scales: Tensor,  # [N, 3]
    template_opacities: Optional[Tensor],  # [N] or None
    viewmats: Tensor,  # [C, 4, 4]
    Ks: Tensor,  # [M, 3, 3]
    width: int,
    height: int,
    ...
    """Packed projection for batched Gaussians with shared scale/opacity template.

    This is an inference-only variant of :func:`fully_fused_projection` for the
    runtime batched renderer. It projects per-instance centers/rotations while
    reading scales and opacities from one shared template.
    """
    assert (
        camera_model != "ftheta"
    ), "ftheta camera is only supported via UT, please set with_ut=True in the rasterization()"
    assert means.dim() == 3 and means.shape[-1] == 3, means.shape
    B, N = means.shape[:2]
    assert quats.shape == (B, N, 4), quats.shape
    assert template_scales.shape == (N, 3), template_scales.shape
    if template_opacities is not None:
        assert template_opacities.shape == (N,), template_opacities.shape
        template_opacities = template_opacities.contiguous()
    assert viewmats.dim() == 3 and viewmats.shape[-2:] == (4, 4), viewmats.shape
    assert Ks.dim() == 3 and Ks.shape[-2:] == (3, 3), Ks.shape
    C = viewmats.shape[0]
    M = Ks.shape[0]
    if intrinsic_ids is None:
        if M == 1:
            intrinsic_ids = torch.zeros((C,), device=means.device, dtype=torch.long)
        elif M == C:
            intrinsic_ids = torch.arange(C, device=means.device, dtype=torch.long)
        else:
            raise ValueError(
                "intrinsic_ids is required when Ks has neither one shared "
                f"intrinsic nor one intrinsic per view: Ks.shape={tuple(Ks.shape)}, "
                f"viewmats.shape={tuple(viewmats.shape)}"
            )
    else:
        assert intrinsic_ids.shape == (C,), intrinsic_ids.shape
        intrinsic_ids = intrinsic_ids.to(device=means.device, dtype=torch.long)
    ...
    (
        ...
        conics,
        compensations,
    ) = _make_lazy_cuda_func("projection_ewa_3dgs_shared_template_packed_fwd")(
        means.contiguous(),
        quats.contiguous(),
        template_scales.contiguous(),
        template_opacities,
        viewmats.contiguous(),
        Ks.contiguous(),
        intrinsic_ids.contiguous(),
        width,
        height,
        eps2d,
        near_plane,
...
@torch.no_grad()
def fully_fused_projection_shared_template_sh_rgb(
    means: Tensor,  # [B, N, 3]
    quats: Tensor,  # [B, N, 4]
    template_scales: Tensor,  # [N, 3]
    template_opacities: Tensor,  # [N]
    template_shs: Tensor,  # [N, 16, 3]
    viewmats: Tensor,  # [C, 4, 4]
    Ks: Tensor,  # [M, 3, 3]
    ...
    """Shared-template packed projection that also materializes RGB/opacity.

    This inference-only fast path targets the batched renderer's common
    template-SH case: SH degree 3 coefficients shared across instances.
    """
    assert (
        camera_model != "ftheta"
    ), "ftheta camera is only supported via UT, please set with_ut=True in the rasterization()"
    ...
    assert template_scales.shape == (N, 3), template_scales.shape
    assert template_opacities.shape == (N,), template_opacities.shape
    assert template_shs.shape == (N, 16, 3), template_shs.shape
    assert viewmats.dim() == 3 and viewmats.shape[-2:] == (4, 4), viewmats.shape
    assert Ks.dim() == 3 and Ks.shape[-2:] == (3, 3), Ks.shape
    C = viewmats.shape[0]
    assert camera_centers.shape == (C, 3), camera_centers.shape
    M = Ks.shape[0]
    if intrinsic_ids is None:
        if M == 1:
            intrinsic_ids = torch.zeros((C,), device=means.device, dtype=torch.long)
    ...
    (
        ...
        "projection_ewa_3dgs_shared_template_sh_rgb_packed_fwd"
    )(
        means.contiguous(),
        quats.contiguous(),
        template_scales.contiguous(),
        template_opacities.contiguous(),
        template_shs.contiguous(),
        viewmats.contiguous(),
        Ks.contiguous(),
        intrinsic_ids.contiguous(),
        camera_centers.contiguous(),
        width,
        height,
        eps2d,
        near_plane,
...
def isect_tiles(
    ...
    ellipse_tile_filter: bool = False,
    profiler=None,
    ...
    Args:
        ...
        conics: Optional conic coefficients used by ellipse_tile_filter.
        opacities: Optional effective opacities used by ellipse_tile_filter.
        image_width: Image width in pixels. Required if ellipse_tile_filter is True.
        image_height: Image height in pixels. Required if ellipse_tile_filter is True.
        ellipse_tile_filter: If True, conservatively skip AABB tiles whose pixel-center
            rectangle cannot overlap the opacity-thresholded Gaussian ellipse.
        profiler: Optional render component profiler that can receive detailed timing and workload stats.
    ...
    if ellipse_tile_filter:
        assert conics is not None, "conics is required when ellipse_tile_filter is True"
        assert (
            opacities is not None
        ), "opacities is required when ellipse_tile_filter is True"
        assert (
            image_width is not None and image_height is not None
        ), "image_width and image_height are required when ellipse_tile_filter is True"
        assert conics.shape == means2d.shape[:-1] + (3,), conics.shape
        assert opacities.shape == depths.shape, opacities.shape
        conics = conics.contiguous()
        opacities = opacities.contiguous()
    else:
        conics = None
        opacities = None
        image_width = tile_width * tile_size if image_width is None else image_width
        image_height = tile_height * tile_size if image_height is None else image_height

    profile_intersect = profiler is not None and hasattr(profiler, "record_value")
    intersect_func_name = (
        "intersect_tile_profiled" if profile_intersect else "intersect_tile"
    )
    result = _make_lazy_cuda_func(intersect_func_name)(
        means2d.contiguous(),
        radii.contiguous(),
        depths.contiguous(),
        conics,
    ...
    if profile_intersect:
        tiles_per_gauss, isect_ids, flatten_ids, timings = result
        timing_names = (
        ...
        for name, value in zip(timing_names, timings.tolist()):
            profiler.record_value(name, float(value))

        if packed:
            visible_gaussians = int(tiles_per_gauss.numel())
        else:
            visible_gaussians = int(torch.count_nonzero(tiles_per_gauss).item())
        total_tile_intersections = int(isect_ids.numel())
        if tiles_per_gauss.numel() > 0:
            max_tiles = int(tiles_per_gauss.max().item())
        else:
            max_tiles = 0
        profiler.record_value("isect_visible_gaussians", float(visible_gaussians))
        profiler.record_value(
            "isect_total_tile_intersections", float(total_tile_intersections)
        )
        profiler.record_value(
            "isect_avg_tiles_per_gaussian",
            float(total_tile_intersections) / float(max(visible_gaussians, 1)),
        )
        profiler.record_value("isect_max_tiles_per_gaussian", float(max_tiles))
    else:
...
def rasterize_to_pixels(
    ...
    if backgrounds is not None:
        if packed:
            packed_image_dims = isect_offsets.shape[:-2]
            if backgrounds.shape == (channels,):
                backgrounds = backgrounds.expand(packed_image_dims + (channels,))
            else:
                assert backgrounds.shape == packed_image_dims + (channels,), (
                    backgrounds.shape,
                    packed_image_dims + (channels,),
                )
        else:
...
@torch.no_grad()
def rasterize_to_pixels_inference(
    means2d: Tensor,  # [..., N, 2] or [nnz, 2]
    conics: Tensor,  # [..., N, 3] or [nnz, 3]
    colors: Tensor,  # [..., N, channels] or [nnz, channels]
    opacities: Tensor,  # [..., N] or [nnz]
    ...
    """Rasterizes Gaussians to pixels without saving backward-only state."""
    ...
    render_colors, render_alphas = _make_lazy_cuda_func(
        "rasterize_to_pixels_3dgs_fwd_inference"
    )(
        means2d.contiguous(),
        conics.contiguous(),
        colors.contiguous(),
