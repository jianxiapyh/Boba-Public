...
std::tuple<
    ...
    at::Tensor,
    at::Tensor>
projection_ewa_3dgs_shared_template_packed_fwd(
    ...
    if (B && C && N) {
        at::Tensor block_cnts =
            at::empty({nrows * blocks_per_row}, opt.dtype(at::kInt));
        launch_projection_ewa_3dgs_shared_template_packed_fwd_kernel(
            // inputs
            means,
            quats,
            scales,
            ...
            Ks,
            intrinsic_ids,
            c10::nullopt, // template_shs
            c10::nullopt, // camera_centers
            image_width,
            image_height,
            eps2d,
            near_plane,
            ...
            calc_compensations ? at::optional<at::Tensor>(at::empty({1}, opt))
                               : c10::nullopt,
            c10::nullopt, // packed_opacities
            c10::nullopt  // packed_colors
        );
        block_accum = at::cumsum(block_cnts, 0, at::kInt);
        nnz = block_accum[-1].item<int32_t>();
    } else {
...
projection_ewa_3dgs_shared_template_sh_rgb_packed_fwd(
    ...
    const at::Tensor template_shs,   // [N, 16, 3]
    const at::Tensor viewmats,       // [C, 4, 4]
    const at::Tensor Ks,             // [M, 3, 3]
    const at::Tensor intrinsic_ids,  // [C]
    const at::Tensor camera_centers, // [C, 3]
    const uint32_t image_width,
    const uint32_t image_height,
    const float eps2d,
    const float near_plane,
    ...
    CHECK_INPUT(template_shs);
    CHECK_INPUT(viewmats);
    CHECK_INPUT(Ks);
    CHECK_INPUT(intrinsic_ids);
    CHECK_INPUT(camera_centers);

    uint32_t N = means.size(-2);
    uint32_t C = viewmats.size(-3);
    uint32_t B = means.numel() / (N * 3);
    TORCH_CHECK(template_shs.size(0) == N && template_shs.size(1) == 16 &&
                template_shs.size(2) == 3,
                "template_shs must have shape [N, 16, 3]");
    TORCH_CHECK(opacities.size(0) == N, "opacities must have shape [N]");
    TORCH_CHECK(camera_centers.size(0) == C && camera_centers.size(1) == 3,
                "camera_centers must have shape [C, 3]");
    auto opt = means.options();
    ...
    at::Tensor depths = at::empty({nnz}, opt);
    at::Tensor conics = at::empty({nnz, 3}, opt);
    at::Tensor packed_opacities = at::empty({nnz}, opt);
    at::Tensor packed_colors = at::empty({nnz, 3}, opt);
    at::Tensor compensations;
    if (calc_compensations) {
        compensations = at::zeros({nnz}, opt);
    }
    ...
    if (nnz) {
        launch_projection_ewa_3dgs_shared_template_packed_fwd_kernel(
            ...
            Ks,
            intrinsic_ids,
            at::optional<at::Tensor>(template_shs),
            at::optional<at::Tensor>(camera_centers),
            image_width,
            image_height,
            eps2d,
            near_plane,
