...
template <typename scalar_t, bool SHARED_TEMPLATE>
__global__ void projection_ewa_3dgs_packed_fwd_kernel(
    ...
    const scalar_t *__restrict__ viewmats, // standard: [B, C, 4, 4], shared-template: [C, 4, 4]
    const scalar_t *__restrict__ Ks,       // standard: [B, C, 3, 3], shared-template: [M, 3, 3]
    const int64_t *__restrict__ intrinsic_ids, // [C] optional shared-template mapping
    const scalar_t *__restrict__ template_shs,    // [N, 16, 3] optional
    const scalar_t *__restrict__ camera_centers,  // [C, 3] optional
    ...
    scalar_t *__restrict__ packed_opacities, // [nnz] optional
    scalar_t *__restrict__ packed_colors     // [nnz, 3] optional
) {
    ...
    if (valid) {
        ...
        means += bid * N * 3 + gid * 3;
        viewmats += (SHARED_TEMPLATE ? cid * 16 : bid * C * 16 + cid * 16);
    ...
    if (valid) {
        ...
        if (covars != nullptr) {
            ...
            quats += bid * N * 4 + gid * 4;
            scales += (SHARED_TEMPLATE ? gid * 3 : bid * N * 3 + gid * 3);
    ...
    if (valid) {
        ...
        if (opacities != nullptr) {
            float opacity = opacities[SHARED_TEMPLATE ? gid : col_idx];
    ...
    if (block_cnts != nullptr) {
        ...
        if (valid) {
            ...
            // write to outputs
            const bool output_packed_attrs =
                packed_opacities != nullptr || packed_colors != nullptr;
            batch_ids[thread_data] = bid;
            camera_ids[thread_data] = cid;
            gaussian_ids[thread_data] =
                (SHARED_TEMPLATE && output_packed_attrs) ? bid * N + gid : gid;
            ...
            if (packed_opacities != nullptr) {
                float packed_opacity = opacities[gid];
                if (compensations != nullptr) {
                    packed_opacity *= compensation;
                }
                packed_opacities[thread_data] =
                    static_cast<scalar_t>(packed_opacity);
            }
            if (packed_colors != nullptr) {
                vec3 dir =
                    glm::make_vec3(means) -
                    glm::make_vec3(camera_centers + cid * 3);
                scalar_t rgb[3];
                const scalar_t *coeffs = template_shs + gid * 16 * 3;
                sh_coeffs_to_color_fast(3, 0, dir, coeffs, rgb);
                sh_coeffs_to_color_fast(3, 1, dir, coeffs, rgb);
                sh_coeffs_to_color_fast(3, 2, dir, coeffs, rgb);
                packed_colors[thread_data * 3] =
                    static_cast<scalar_t>(fmaxf(static_cast<float>(rgb[0]) + 0.5f, 0.0f));
                packed_colors[thread_data * 3 + 1] =
                    static_cast<scalar_t>(fmaxf(static_cast<float>(rgb[1]) + 0.5f, 0.0f));
                packed_colors[thread_data * 3 + 2] =
                    static_cast<scalar_t>(fmaxf(static_cast<float>(rgb[2]) + 0.5f, 0.0f));
            }
        }
            ...
            projection_ewa_3dgs_packed_fwd_kernel<scalar_t, false>
                <<<grid,
                   ...
                   at::cuda::getCurrentCUDAStream()>>>(
                    ...
                    nullptr, // intrinsic_ids
                    nullptr, // template_shs
                    nullptr, // camera_centers
                    image_width,
                    image_height,
                    eps2d,
                    near_plane,
                    ...
                    compensations.has_value()
                        ...
                        : nullptr,
                    nullptr, // packed_opacities
                    nullptr  // packed_colors
                );
        }
    );
}

void launch_projection_ewa_3dgs_shared_template_packed_fwd_kernel(
    // inputs
    ...
    const at::optional<at::Tensor> opacities, // [N] optional
    const at::Tensor viewmats,             // [C, 4, 4]
    const at::Tensor Ks,                   // [M, 3, 3]
    const at::Tensor intrinsic_ids,        // [C]
    const at::optional<at::Tensor> template_shs, // [N, 16, 3] optional
    const at::optional<at::Tensor> camera_centers, // [C, 3] optional
    const uint32_t image_width,
    const uint32_t image_height,
    const float eps2d,
    const float near_plane,
    ...
    at::optional<at::Tensor> conics,       // [nnz, 3]
    at::optional<at::Tensor> compensations, // [nnz] optional
    at::optional<at::Tensor> packed_opacities, // [nnz] optional
    at::optional<at::Tensor> packed_colors     // [nnz, 3] optional
) {
    uint32_t N = means.size(-2);          // number of template gaussians
    uint32_t C = viewmats.size(-3);       // number of cameras
    uint32_t B = means.numel() / (N * 3); // number of batches
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means.scalar_type(),
        "projection_ewa_3dgs_shared_template_packed_fwd_kernel",
        [&]() {
            projection_ewa_3dgs_packed_fwd_kernel<scalar_t, true>
                <<<grid,
                   ...
                   at::cuda::getCurrentCUDAStream()>>>(
                    ...
                    viewmats.data_ptr<scalar_t>(),
                    Ks.data_ptr<scalar_t>(),
                    intrinsic_ids.data_ptr<int64_t>(),
                    template_shs.has_value()
                        ? template_shs.value().data_ptr<scalar_t>()
                        : nullptr,
                    camera_centers.has_value()
                        ? camera_centers.value().data_ptr<scalar_t>()
                        : nullptr,
                    image_width,
                    image_height,
                    eps2d,
                    ...
                    compensations.has_value()
                        ? compensations.value().data_ptr<scalar_t>()
                        : nullptr,
                    packed_opacities.has_value()
                        ? packed_opacities.value().data_ptr<scalar_t>()
                        : nullptr,
                    packed_colors.has_value()
                        ? packed_colors.value().data_ptr<scalar_t>()
                        : nullptr
                );
        }
    );
