...
template <uint32_t CDIM, typename scalar_t, bool WRITE_LAST_IDS>
...
void launch_rasterize_to_pixels_3dgs_fwd_kernel(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dgs_fwd_kernel<CDIM, float, true>,
...
template <uint32_t CDIM>
void launch_rasterize_to_pixels_3dgs_fwd_inference_kernel(
    // Gaussian parameters
    const at::Tensor means2d,   // [..., N, 2] or [nnz, 2]
    const at::Tensor conics,    // [..., N, 3] or [nnz, 3]
    const at::Tensor colors,    // [..., N, channels] or [nnz, channels]
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dgs_fwd_kernel<CDIM, float, false>,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            shmem_size
        ) != cudaSuccess) {
        AT_ERROR(
...
#define __INS__(CDIM)                                                          \
    ...
    template void                                                              \
    launch_rasterize_to_pixels_3dgs_fwd_inference_kernel<CDIM>(                \
        const at::Tensor means2d,                                              \
        const at::Tensor conics,                                               \
        const at::Tensor colors,                                               \
        const at::Tensor opacities,                                            \
