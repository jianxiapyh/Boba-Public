...
try:
    ...
    distF = None
    _DIST_F_IMPORT_ERROR = exc
else:
    _DIST_F_IMPORT_ERROR = None


def _require_distF() -> None:
    if distF is None:
        raise RuntimeError(
            "Differentiable distributed collectives require "
            "torch.distributed.nn.functional, but the active torch build "
            "does not provide it."
        ) from _DIST_F_IMPORT_ERROR
