...
class InvPhyTrainerWarp:
    ...
    def test(self, model_path=None):
        ...
        video_path = f"gaussian_output_dynamic/{cfg.run_name}/inference.mp4"
        save_path =  f"gaussian_output_dynamic/{cfg.run_name}/inference.pkl"
        self.visualize_sim(
            save_only=True,
    ...
    def visualize_sim(
        ...
        if save_trajectory:
            #pyh force create the directory if does not exist
    ...
    def update_frame(self, frame, pressed_keys, replay_experiment_trace=False):
        result = frame.copy()

        #pyh only enable during record mode
        if not replay_experiment_trace:
    ...
    def interactive_playground(
        ...
        if replay_experiment_trace:
            current_target = self.dataset.controller_points[0].clone()
        else:
        ...
        if save_eval_artifacts:
            ...
            num_views = 1
        render_views = [(0, view)]
        if save_eval_artifacts and num_views > 1:
            for view_idx in range(1, num_views):
                render_views.append(
                    (
                        view_idx,
                        self._create_gs_view(
                            cfg.w2cs[view_idx],
        ...
        if n_ctrl_parts > 1:
            ...
            for i in range(n_ctrl_parts):
                masks_ctrl_pts.append(torch.from_numpy(cluster_labels == i))
        ...
        if output_dir is not None and save_eval_artifacts:
            eval_render_paths = {
        ...
        try:
            while True:
                ...
                if save_eval_artifacts and frame_count == 0:
                    x = wp.to_torch(
                    ...
                    if traj_frames_cpu is not None:
                ...
                frame_timer.start()
                frame = overlay.clone()
                frame_setup_time = frame_timer.stop()
                ...
                if save_eval_artifacts and num_views > 1:
                    for view_idx, extra_view in render_views[1:]:
                        additional_renderings[view_idx] = render_gaussian(
                            extra_view, gaussians, None, background
                        )["render"]
                ...
                frame = self.update_frame(frame, self.pressed_keys, replay_experiment_trace)
                ...
                if frame_count > 1:
                    ...
                    fps_history.append(1.0 / total_time)

                if output_dir is not None and save_eval_artifacts:
                    torchvision.utils.save_image(
                        rendering,
                        os.path.join(
                            eval_render_paths[0], "{0:05d}".format(frame_count) + ".png"
                        ),
                    )
                    for view_idx, extra_rendering in additional_renderings.items():
                        torchvision.utils.save_image(
                            ...
                            os.path.join(
                                ...
                                "{0:05d}".format(frame_count) + ".png",
                            ),
                        )
                    save_path = os.path.join(
                        view_render_path, f"{frame_count:05d}.png"
                    )
                ...
                if replay_experiment_trace:
                    if frame_count < self.dataset.frame_len:
                        current_target = self.dataset.controller_points[frame_count].clone()
                    else:
                    ...
                    if n_ctrl_parts == 1:
                        ...
                        for i in range(n_ctrl_parts):
                            ...
                            if masks_ctrl_pts[i].sum() > 0:
                                current_target[masks_ctrl_pts[i]] += delta
                            else:
                                current_target += delta
                            if i == 0:
                                self.hand_left_pos += delta
                            elif i == 1:
                                self.hand_right_pos += delta
                        ...
                        json.dump(
                            {
                                ...
                                "average_total_frame_time_ms": float(
            ...
            if traj_frames_cpu is not None:
                ...
                with open(traj_save_path, "wb") as file:
                    pickle.dump(vertices.numpy(), file)
                print(
                    f"[Saved] inference.pkl -> {traj_save_path}, shape={tuple(vertices.shape)}"
                )
